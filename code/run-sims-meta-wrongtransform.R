...
# Adjusting sim_scenarios
sim_scenarios <- sim_scenarios[sim_scenarios[, 'estimand'] != 'm', ] 

if (n_cores > 1){
  registerDoParallel(cores=n_cores)
}
res_all_ci_wrongtransform <- res_all_true_wrongtransform <- vector(mode = 'list', length = nrow(sim_scenarios))
...
for (m in 1:nrow(sim_scenarios)){
  ...
  res_temp <- foreach(t = 1:n_reps) %dorng%{
    ...
    res_ci[1, mycol] <- extract_pooled_mean(fit_ci, estimand = other_estimand)
    res_true[1, mycol] <- extract_pooled_mean(fit_true, estimand = other_estimand)
  ...
  res_all_ci_wrongtransform[[m]] <- res_ci
  res_all_true_wrongtransform[[m]] <- res_true
}

save.image('../results/sim-res-meta-wrongtransform.RData')
